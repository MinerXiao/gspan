...
public final class Miner {
    ...
    /**
     * @param <NodeType> the type of the node labels (will be hashed and checked with
     *                   .equals(..))
     * @param <EdgeType> the type of the edge labels (will be hashed and checked with
     *                   .equals(..))
     * @param graphs
     * @param settings
    ...
    public static <NodeType, EdgeType> Collection<Fragment<NodeType, EdgeType>> mine(
        ...
        Collection<Fragment<NodeType, EdgeType>> ret = settings.strategy.search(settings.algorithm);
        ...
        final FragmentFilter<NodeType, EdgeType> filter = LocalEnvironment.env(settings.strategy).filter;
        if (filter != null) {
            if (INFO) {
