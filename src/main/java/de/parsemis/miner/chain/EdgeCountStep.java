 ...
 *            .equals(..))
 */
public class EdgeCountStep<NodeType, EdgeType> extends
     ...
     *            reported
     */
    public EdgeCountStep(final MiningStep<NodeType, EdgeType> next,
    ...
    public void call(final SearchLatticeNode<NodeType, EdgeType> node,
        ...
        final int size = node.toHPFragment().toHPGraph().getEdgeCount();
        if (size < min) {
            node.store(false);
        }
        if (size > max) {
